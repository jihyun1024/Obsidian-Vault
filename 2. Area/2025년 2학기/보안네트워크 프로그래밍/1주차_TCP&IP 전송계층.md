...
- IP 주소
	...
	- IPv4는 8bit 단위로 점으로 구분하며, IPv6는 16bit 단위로 콜론으로 구분해 표시한다. 
	
- 포트 번호
	- 인터넷 통신의 종착점을 나타내는 식별자이다. 
	- TCP, UDP는 포트 번호로 부호 없는 16bit 정수를 사용하기 때문에 0에서 65535까지의 범위에 있는 값들이 가능하다. 
	- 영역별로 포트 번호를 배정해 두었으며, 값은 아래와 같다. 
		- ![[Pasted image 20250918045706.png]]

- 도메인 이름
	- IP 주소에 대한 기억하기 쉬운 별명으로, `kookmin.ac.kr`등등이다. 
	- 따라서, 실제로 통신할 때는 IP 주소로 해당 값을 변환해야 하며, DNS(Domain Name Server)를 통해 이에 해당하는 IP 주소를 알 수 있다. 

IP 주소와 포트 번호와의 관계를 그림으로 간단하게 나타내면 다음과 같다. 
![[Pasted image 20250918045744.png]]

### 소켓의 기본 개념
소켓은 다음 세 가지의 관점으로 볼 수 있다. 
#### 데이터 타입
파일 디스크립터나 파일 핸들(예시: [[Ragnar Locker 파일 암호화 알고리즘 파악#개요 및 함수 확인|Ragnar Locker 랜섬웨어에서의 파일 핸들 다루기]])과 유사한 개념으로, 생성과 설정 과정이 끝나면 운영체제의 통신 관련 정보를 참조해 다양한 작업을 편리하게 할 수 있는 데이터 타입으로 볼 수 있다. 
![[Pasted image 20250918050520.png]]

#### 통신 중단점
응용 프로그램은 자신의 소켓이 상대의 소켓과 직접 연결된 것으로 간주하고 데이터를 주고 받을 수 있어 세부적인 네트워크 절차를 굳이 고려하지 않고도 네트워크를 구현할 수 있다. 

이는 네트워크에 대한 프로그래밍을 용이하게 할 수 있다.
![[Pasted image 20250918050640.png]]

#### 네트워크 프로그래밍 인터페이스
TCP/IP 프로토콜에서 일반적으로 응용 계층과 전송 계층 사이에 위치하는 것으로 간주한다. 
![[Pasted image 20250918050716.png]]

---
## 중요한 단어
1. TCP/IP 프로토콜의 구조와 각각의 역할
2. 소켓의 개념