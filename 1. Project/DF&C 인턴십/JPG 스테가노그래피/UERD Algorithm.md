...
어떤 특정 DCT 계수를 변경할 때 발생하는 **왜곡 비용(distortion cost)** 을 수치화한 함수
역할: 
- 은닉 위치 선택 기준 제공
- 수정으로 인한 확률 분포 변화까지 고려해 탐지 저항성 보장
...
- **Block 기반 비용 (ρᵢⱼ,block)**  
  ...
  - 클수록 텍스처가 많아 삽입해도 티가 잘 안남 → block 비용이 작아짐
...
- 각 계수에 대해  
  ...
  - Block 기반 비용 ($ρᵢⱼ,block$)  
	  ...
	  - 평탄한 블록(DC, 저주파 쪽)에서는 비용이 커 변경이 감소
	  - 복잡도가 높은 블록(texture 영역)에서는 비용이 작아 변경이 증가
  - 최종 비용 $ρᵢⱼ$ = 두 요소의 곱

### 단계 4: 최소왜곡 임베딩
- Syndrome-Trellis Codes (STC) 이용  
- 주어진 페이로드를 만족하면서 총 비용 $Σρ$ 최소화
- 각 계수에 대해 최종 비용 $ρᵢⱼ$ 을 계산한 경우, 비용이 낮은 계수에 대해:
	- 메시지 비트 $m_i$를 은닉하려면 계수 $c_i$를 +1, -1, 또는 그대로 해서 변화 (이 과정이 은닉)
