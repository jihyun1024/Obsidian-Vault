...
세 번의 `lockbit.0B4650` 함수를 호출하는 경우 중 맨 처음 `CryptImportKey` 함수를 호출하는 경우에 대해 F7으로 들어가면서 분석해 보자. 함수 프롤로그를 포함한 부분은 다음과 같다. 
![[Pasted image 20251116060018.png]]
이 때, 64bit 코드는 [[리버스 엔지니어링 입문#x64 호출 규약|FASTCALL 호출 규약]]을 사용하기 때문에 처음 4개의 매개변수를 함수에 레지스터(rcx, rdx, r8, r9)로 전달하고 나머지 매개변수는 스택에 저장한다. 따라서, 함수나 API의 호출을 디버깅하는 동안 조사하고자 하는 매개변수에 따라 레지스터 또는 스택을 조사해야 한다. 
따라서, rcx, rdx, r8, r9 레지스터를 보면 각각 값들이 함수의 인수가 된다는 것을 확인 가능하다.
(FASTCALL 호출 규약도 설명할 수 있도록 하자)

현재 rcx에는 `0x1C737EEF89F20000`, rdx에는 `0x10`, r8에는 `0x70D2C0E4`, r9에는 `0x37` 값이 있는 상태이다. 따라서, 해당 값들이 함수의 인수가 된다는 것을 확인 가능하다.

이후에 쭉 실행을 하다가 `lockbit.0B38D0` 함수로 도착하게 되며, 그 전 명령어는 `xor ecx, ecx` 으로 추가적인 인수 입력은 없이, rdx 레지스터에 `0x10`이, r8 레지스터에 `0x70D2C0E4`가 있어 그 둘이 인수인 것으로 판단했다. ![[Pasted image 20251116082851.png]]

### 암호화 API 호출: `lockbit.0B38D0`
그렇다면, `lockbit.0B38D0`에서는 무엇을 하는지 F7으로 타고 들어가 분석해 보자. 
