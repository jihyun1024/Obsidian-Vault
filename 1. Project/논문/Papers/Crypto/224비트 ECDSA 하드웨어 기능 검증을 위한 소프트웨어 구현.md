...
---
### 목표 ECDSA 파라미터
타원곡선 전자서명을 구현하기 위해 필요한 파라미터의 값은 다음과 같다.

![[Pasted image 20260216205353.png]]

이는 타원곡선 $E:\ y^2 = x^3 + ax + b$에서 $a$값과 $b$값, 소수 $p$의 값, Generator $G$, Order $n$의 값을 알아야 ECDSA를 구현할 수 있기 때문에 지정한 것으로, 이 논문에서 구현한 소프트웨어는 224비트의 ECDSA를 선택했고, 이에 맞춰 소수체 $p_{224} = 2^{224} - 2^{96} + 1$을 사용한다. 


### 목표 하드웨어 기본 구조
소프트웨어를 설계하기에 앞서서, 목표하는 하드웨어의 기본 구조에 대해 알아봐야 한다. 해당 하드웨어의 특징은 다음과 같다. 

- 4비트 단위 덧셈 수행
- 한 비트씩 순차적 곱셈, 오른쪽 비트부터 2배 연산과 덧셈을 반복하는 스칼라 R2L 곱셈

이에 따라 하드웨어 구조를 나타낸 논문에서의 그림은 다음과 같다. 

<br>

**1. 덧셈 내부 구조 (k비트 단위 덧셈)**
![[Pasted image 20260216210656.png|350]]

224비트를 k 비트 단위로 나눠서 입력하면, 덧셈을 한 후 캐리 값과 연산결과 값을 출력한다. 캐리 값은 상위 비트로 보내며, 다음과 같이 동작한다. 

- 캐리 값이 1이면, $C + D + 1$ 연산을 선택
- 캐리 값이 0이면, $C+ D$ 연산을 선택

이런 과정은 캐리 값을 받아 선택하는 과정이 존재하지만, 실제 내부 연산을 병렬로 처리하여 하드웨어적으로 구현했을 때 속도의 향상을 기대할 수 있다. 

<br>

**2. k비트 단위 병렬구조를 통한 224비트 덧셈기 내부 하드웨어 구조**
![[Pasted image 20260216211357.png|600]]

224비트 덧셈기 하드웨어 구조를 살펴보면 다음과 같다. 

1. k비트씩 쪼개 Four Bit Adder 모듈에 병렬로 처리한다. 
2. 출력되는 캐리 값은 상위 비트에 보낸다.
3. u 값은 k비트씩 나온 결과를 전부 붙이고 **MSB의 캐리 값을 포함해 225비트를 출력**한다.

<br>

**3. 타원곡선 위 점 연산을 R2L 방식으로 반복하는 스칼라 곱셈식**
이에 먼저, 목표 하드웨어의 유한체 곱셈은 다음과 같이 계산할 수 있다. 

$$
\begin{aligned}
AB \bmod P
&= A(b_{n-1}, b_{n-2}, \cdots, b_1, b_0) \bmod P \\[6pt]
&= b_{n-1}(A2^{\,n-1})
 + b_{n-2}(A2^{\,n-2})
 + \cdots
 + b_1(A2)
...
계산 과정은 224비트의 $A$, $B$를 입력하면 $B$의 한 비트를 $A$와 곱셈을 한다. 

- $A = 1$인 경우, Left Shift 연산과 덧셈을 한 후 다시 반복
- $A =0$인 경우, 덧셈을 한 후 다시 반복

이러한 연산 과정을 통해서 $A$와 $B$의 곱셈 결과를 얻을 수 있으며, **이를 통해 구현할 수 있는 타원곡선 상의 점 연산을 수행하는 스칼라 곱셈식**은 다음과 같다. 

$$
\begin{aligned}
dP
&= (d_{n-1}, d_{n-2}, \cdots, d_0)P \\[6pt]
&= \bigl(
      d_0P
    + d_1 2P
    + d_2 2(2P)
...
이를 소수체 $GF(p)$ 위의 타원곡선 $E: y^2 = x^3 +ax+b$ 위의 점 $P$에 대해 스칼라 $d$로 곱셈을 하여 $dP$를 구하는 병렬 알고리즘으로 다음과 같이 설계할 수 있다. 

![[Pasted image 20260216215035.png|400x250]]


### ECDSA 하드웨어 맞춤형 소프트웨어 설계 및 구현
목표 하드웨어 구조의 특징 중 하나인 k비트로 나누어 입력되는 방식에서, 선택한 4비트 단위에 맞춰 검증 소프트웨어의 모든 모듈이 4비트 단위로 동작하도록 구현되었다. 

224비트의 파라미터 값이 들어오면 `char`형 변수에 4비트 단위로 나눠서 들어오게 되는데, 연산 과정에서 `carry` 값을 위해 상위 1비트를 추가로 사용해 총 5비트를 사용하고 연산과정이 끝나면, 상위 1비트의 `carry` 값은 삭제하여 4비트 단위로 출력한다. 

내부 모듈 중 가장 많이 사용되며 기본적인 함수는 다음 두 개이다.

- `void INTEGER_Adder()`
- `void INTEGER_Sub()`

두 함수는 앞서 설명한 224비트 덧셈기 하드웨어 구조를 사용하며, 각각 테스트벡터를 제공한다. 

또한 위에서 언급한 두 모듈을 사용해 곱셈, 역원, 모듈로 연산이 가능하며, 이 연산 모듈의 테스트벡터 제공 함수는 다음과 같다. 
...
하위 모듈을 사용해 타원곡선 점 연산을 구현할 수 있으며, 각각 테스트벡터를 제공한다. 또한, 타원곡선 위의 두 점 덧셈과 한 점의 두 배 연산을 사용해 스칼라 곱셈을 구현할 수 있다. 

- `void DP_dP_2P()`
- `void DP_dP_module()`
- `void DP_dP_PplusQ()`

다음은 ECDSA 전자서명 서명과 검증 함수로, 서명 생성과 검증 테스트벡터를 제공한다. 

- `void ECDSA_ECDSA_Sign()`
- `void ECDSA_ECDSA_VERIFY()`

프로그램은 C++로 구현했으며, 아래 그림은 프로그램의 초기 화면이다. 프로그램은 앞서 설명한 함수들을 사용해 내부 모듈을 검증할 수 있는 테스트벡터를 제공하도록 구성되어 있고, 많은 연산을 거치는 모듈은 연산 중간값을 텍스트로 제공한다. 

![[Pasted image 20260216222838.png|400]]


## 6. ECDSA 하드웨어 내부 모듈 검증
---
이 절에서는 앞서 설명한 하드웨어 구현 검증의 필요성에 따라 타원곡선 전자서명의 내부 모듈인 유한체 사칙연산부터 타원곡선 점 연산, 스칼라 곱셈, 전자서명 서명생성 및 서명검증 모듈의 검증과정을 거친다. 

![[Pasted image 20260216225541.png]]

### 하드웨어 검증 방법
검증 흐름은 다음과 같다. 

1. 해시함수를 사용해 난수를 생성하여 사용되는 입력값 A와 B를 각 모듈에 동작 후 C 생성
2. 입력값 테스트벡터 A.txt와 B.txt를 하드웨어에 전달
3. 하드웨어에서 테스트벡터 두 개를 가지고 모듈을 동작시켜 결과값 D를 생성
4. 하드웨어 결과값 D를 D.txt 파일로 받음
5. 소프트웨어 결과값 C를 C.txt 파일로 받음
6. 두 결과값 C와 D를 비교하여 검증

이 순서를 여러 번 반복해 테스트하여 하드웨어 내부 모듈을 검증한다. 

![[Pasted image 20260216225937.png]]

### 유한체 사칙연산 모듈 검증
ECDSA의 하위 모듈인 유한체 사칙연산 모듈은 총 10가지의 모듈이 존재하며 다음과 같다. 
...
- UmodN
	...
	- Adder를 사용해 구현
- Multiplier_modP
	- 한 비트씩 곱셈을 통해 유한체 곱셈 계산을 수행하는 모듈
	- Adder와 UmodP를 사용해 구현
- Invertor_modP
	- 유한체 원소 값의 역원 값
	- Adder, UmodP, UsubVmodP, 한 비트 시프트 연산을 사용해 구현
- Invertor_modN
- Multiplier_modN
	- 위의 두 모듈은 각각 앞서 구현한 Invertor_modP, Multiplier_modP 모듈과 비슷한 구조
	- 테스트 벡터 제공 방식도 동일함

10가지의 모듈 중에서 모듈러 연산은 modP와 modN으로 나뉘며, P는 스칼라 곱셈까지의 연산과정에서, N은 전자서명 과정에서 사용된다.

**검증은 위에서 설명한 것처럼, 테스트벡터를 사용해 하드웨어 연산 결과와 소프트웨어 연산 결과를 비교하는 방식**으로 진행한다.

### 스칼라 곱셈 모듈 검증
스칼라 곱셈 모듈은 타원곡선 점 연산 모듈과 하위 모듈을 사용해 타원곡선 위의 점 덧셈과 두 배 연산을 반복하여 구현할 수 있다. 

또한, 스칼라 곱셈은 많은 연산을 거치기 때문에 **연산의 중간 값을 확인할 수 있도록** 구현했다. 
$dP$ 연산 모듈은 224비트의 $d$를 곱셈해야 하므로 224번 반복해 곱셈한다. 따라서, 224번의 연산 라운드를 텍스트 파일에 저장해 중간 값을 검증할 수 있다.

![[Pasted image 20260216232132.png]]


### 타원곡선 점 연산 모듈 검증
앞서 설명한 스칼라 곱셈 모듈이 검증에 실패했을 경우, 내부 모듈인 타원곡선 점 연산 모듈의 검증이 필요하다. 

타원곡선 점 연산 모듈은 유한체 사칙연산 모듈을 사용해 검증할 수 있으며, 타원곡선 위의 두 점 $P$와 $Q$를 이용한 두 가지의 연산인 $2P$ 연산과 $P + Q$ 연산이 있다. 

두 가지 모듈은 전부 내부 모듈인 유한체 사칙연산을 바탕으로 작동하기 때문에 많은 연산 과정을 거치므로, 연산의 중간 값을 확인할 수 있다. 

### SHA224 모듈 검증
SHA224 모듈은 타원곡선 전자서명의 서브 모듈로, 서명 생성과 검증 과정에서 필요하다. 따라서 SHA224 모듈에 필요한 메시지와 해시함수를 취한 결과와 검증 소프트웨어 화면은 다음과 같다.

![[Pasted image 20260216232717.png|600]]

### ECDSA 서명 모듈 검증
앞서 설명한 내부 모듈들이 전부 완성되면 ECDSA 서명 모듈이 구현 가능하며, 서명을 구현할 때에는 UmodN, Invertor_modN, Multiplier_modN 모듈이 필요하다. 

서명 테스트 벡터와 서명 결과, 그리고 검증 소프트웨어 화면은 다음과 같다. 

![[Pasted image 20260216232915.png|600]]

또한, 시뮬레이션 결과를 보면 내부 중간 값을 볼 수 있는데, 서명값과 테스트 벡터의 일치 여부를 확인해 서명 생성 연산과정에서 오류를 찾기 쉽다.

![[Pasted image 20260216233058.png|600]]


### ECDSA 검증 모듈 검증
ECDSA 검증 모듈은 ECDSA 서명 값을 검증하기 위한 모듈로, 앞서 설명한 모든 모듈을 사용하여 구현한다. 검증 모듈을 구현할 때는 **사전에 비밀 키를 공유했다고 가정**한다. 

ECDSA 검증 테스트 벡터와 검증 결과, 그리고 검증 소프트웨어의 화면은 다음과 같다. 

![[Pasted image 20260216233331.png|600]]
