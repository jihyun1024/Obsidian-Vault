...
[[Jadx 메인 문서|Jadx]] 검색창에 코드(대소문자 구분 안 함)만 선택해서 jpg를 검색한 결과 다음과 같이 4개의 검색 결과를 도출하였음. 
                ...
                if (byteArray.length < bytes.length * 0.9f) {
                    ...
                    while (i7 < length2) {  // 전체 바이트를 순회
                        ...
                        while (i12 < i4) { // 바이트의 8비트를 하나하나 꺼내서 처리, 즉 바이트를 비트 단위로 이미지에 삽입 
                            ...
                            int[] iArr3 = iArr2;
                            // 해당 비트를 0으로 클리어 (비트 마스크)
                            int i15 = iArr3[i9] & (i13 ^ 0xffffffffffffffff);
  
                            if (!z2) {
                                i13 = 0x0;
                            }
		                    // z2 = true이면 해당 위치에 1을 삽입, 결국 RGB의 특정 비트를 1 또는 0으로 세팅
                            int i16 = i15 | i13;

							// 마지막 비트는 (1바이트 = 8비트) XOR 방식으로 위치를 섞으며, 즉 bArr[i7] ^ i7의 경우, 바이트의 값과 해당 위치의 값을 XOR
                            if (i12 == 0x7) {
                                i16 = (i16 & 0xfffffffffffffffe) | ((bArr[i7] ^ i7) & 0x1);
                            }

							// 변경된 픽셀값을 다시 배열에 저장 (이미지 내의 픽셀은 수정된 상태)
                            iArr3[i9] = i16;
                            i8 += i14;
                            i9 += i14;

							// 이미지의 가로 길이를 넘으면 다음 세로줄로 이동
                            if (i8 >= width) {
                                i10++;
                                if (i10 >= height) {
                            ...
                            // 다음 비트를 준비하기 위해 바이트를 오른쪽으로 시프트하며, 이 경우 메시지의 1바이트 내에서 비트는 LSB를 읽은 후 그 다음 비트, 그 다음 비트 순으로 MSB 방향으로 읽어 올라감..
                            i11 >>>= 0x1;
                            i12++;
                            i5 = 0x5;
                            iArr2 = iArr3;
...
// 결과 요약
// 1. bArr[]의 각 바이트를 3비트씩 이미지의 픽셀에 (RGB의 LSB에) 삽입
// 2. Red, Green, Blue 순서로 돌아가면서 LSB로 삽입
// 3. 마지막 비트는 XOR 변형으로 위치 기반 무작위화
// 4. 이 방식 덕분에 이미지의 시각적 변화는 거의 없고, 데이터도 잘 숨겨짐!
```
...
18. 메시지의 바이트가 저장된 배열 `bArr[]`을 8비트 단위로 읽고, 각 비트를 `iArr2[]`의 픽셀에 숨긴다. (설명은 코드에)
