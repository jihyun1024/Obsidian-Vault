...
또한, 나중을 위해 게시글 작성 페이지로 이동하는 버튼 또한 24 ~ 26번 라인에 추가해 놓았다. 
...
## 게시글 수정 기능
---
이번에는 게시글을 수정하는 기능을 추가해 볼 것이다. 이를 위해 `GET /posts/<post_id>/edit` 엔드포인트를 구현해 지정된 `post_id`에 해당하는 게시글을 수정할 수 있는 페이지를 반환하는 기능을 구현한다. 그리고 `post_id`에 해당하는 게시글 수정 요청을 실제 처리하는 엔드포인트인 `POST /posts/<post_id>/edit`도 만들어 본다. 


### 게시글 수정하는 함수 추가하기
----
먼저, 데이터베이스 내에 저장된 게시글을 수정하는 함수를 *db.py*에 추가한다. 
...
이 변경된 코드는 다음 SQL query를 실행하는데, 순서대로 인자로 전달받은 `new_title`, `new_content`, `post_id`가 들어가 `posts` 테이블의 데이터를 수정한다.

```
UPDATE posts SET title = ?, content = ? WHERE id = ?
```

또한, `SET title = ?, content = ?`은 테이블에서 데이터를 수정할 때 `title`과 `content` 값만 새로운 값(물음표 자리에 들어가는 값)으로 변경한다는 의미이다. 


### 게시글 수정 엔드포인트 추가하기
---
앞서 *db.py*에 게시글의 고유 ID로 게시글 데이터를 수정할 수 있는 함수를 추가했다면, 이번에는 사용자가 게시글을 수정할 수 있도록 다음 두 개의 엔드포인트를 *app.py*에 만든다. 

- `GET /posts/<post_id>/edit`
	- 경로 매개변수인 `post_id`가 존재하며, 고유번호가 `post_id`에 해당하는 게시글을 수정할 수 있는 페이지를 반환한다. 
	- 반환되는 문서의 이름은 *post_edit.html*로 한다. 
- `POST /posts/<post_id>/edit`
	- 고유번호가 `post_id`에 해당하는 게시글의 수정을 실제로 처리한다. 

```python
 from flask import Flask, redirect, render_template, request, session, url_for
 import os
 from db import init_db, check_account, add_account, get_all_posts, create_post, get_post_by_post_id, update_post
...
---
이번에는 두 개의 HTML 문서를 작성하는데, 각각은 다음과 같다. 

- *post_edit.html*
	- **Post Edit**이라는 문구가 존재하고, **게시글 제목**을 수정할 수 있는 `Title:` 이라는 문구와 입력 폼이 존재하며 **게시글 내용**을 수정할 수 있는 `Content:`라는 문구와 입력 폼이 존재
	- 각각의 입력 폼에는 기존의 제목과 내용이 그대로 있는 상태 
	- 그 아래에는 게시글 수정을 반영할 수 있는 **Edit** 버튼과 게시글 페이지로 돌아갈 수 있는 **Go to Post** 버튼 존재
	- 기대하는 페이지의 형태는 다음 사진과 같음
	![[Pasted image 20260120231717.png]]
- *post_edit_failure.html*
	- **Failed to Edit Post**라는 헤딩 문구가 존재
	- 게시글 목록 페이지로 돌아가는 **Go to Post** 버튼 존재
	- 기대하는 페이지의 형태는 다음 사진과 같음
	![[Pasted image 20260120231958.png]]

먼저, *post_edit.html*의 코드는 아래와 같다. 두 파일 다 별다른 것은 없으니 설명 없이 넘어간다.
...
{% block content %}
  <h2>Post Edit</h2>
  <form action="/posts/{{ post[0] }}/edit" method="post">
    Title:
    <br>
    ...
    Content:
    <br>
    <textarea id="content" name="content">{{ post[2] }}</textarea>
    <br>
    <button>Edit</button>
  </form>
  <br>
  <form action="/posts/{{ post[0] }}">
    <button>Go to Post</button>
  </form>
{% endblock %}
```

또한, *post_edit_failure.html*의 코드는 아래와 같다. 

```html
{% extends 'base.html' %}

{% block content %}
  <h2>Failed to Edit Post</h2>
  <form action="/posts">
    <button>Go to Posts</button>
  </form>
{% endblock %}
...
## 게시글 삭제 기능
---
이번에는 마지막으로 특정 게시글을 삭제하는 기능을 구현해본다. 이를 위해, 두 엔드포인트를 구현해야 하며, 이번은 마지막 기능 구현인 만큼 흐름도 어느 정도 알 것이라 생각한다. 

- `GET /posts/<post_id>/delete`
	- `post_id`에 해당하는 게시글을 삭제할 수 있는 페이지를 반환한다.
	- <u>정말로 삭제할 것인지</u>를 묻는 문구, <u>예, 아니요 버튼</u>을 표시한다.
	- 해당 엔드포인트가 반환하는 HTML 문서의 이름은 *post_delete.html*로 한다.
- `POST /posts/<post_id>/delete`
	- `post_id`에 해당하는 게시글의 삭제를 실제로 처리한다. 


### 게시글 삭제하는 함수 추가하기
---
데이터베이스로부터 특정 게시글을 삭제하는 함수를 *db.py* 파일에 추가한다. 
...
### 게시글 삭제 엔드포인트 추가하기
---
앞서 *db.py*에 게시글을 삭제할 수 있는 함수를 추가했다면, 이번에는 사용자가 게시글을 삭제할 수 있도록 앞에서 설명했던 두 개의 엔드포인트를 함수의 형태로 *app.py*에 추가한다. 
...
이때, `DELETE`라는 메서드도 HTTP 메서드에 있긴 한데, 굳이 `POST` 메서드를 사용해 게시글을 삭제하는 이유는 다음과 같다. 

HTTP 메서드 중 하나인 `DELETE` 메서드도 <u>어떤 데이터를 "삭제"할 때 주로 사용되는 메서드</u>인 것은 맞으나, 브라우저 HTML의 `<form>` 태그는 `GET` 또는 `POST` 메서드만을 지원하기 때문에 `DELETE` 메서드를 사용하기 어렵다. 

브라우저에서 `DELETE` 메서드를 사용하려면 HTML을 넘어서 JavaScript를 사용해야 하는데, 이번 강의에서는 <u>실습과 개발의 편의상 HTML만 사용하고 있어</u> `POST` 메서드를 사용했다.


### 게시글을 삭제할 수 있는 HTML 문서 작업하기
---
이제 게시글 삭제 기능과 관련한 두 개의 HTML 문서를 제작한다. 

1. `GET /posts/<post_id>` 엔드포인트는 게시글 내용을 보여주는 페이지인 *post.html*을 반환하는데, *post.html*에 게시글 삭제 링크를 추가한다.
2. `GET /posts/<post_id>/delete`와 `POST /posts/<post_id>/delete`에서 반환하는 *post_delete.html*과 *post_delete_failure.html*을 만든다.

먼저, *post.html*에 게시글 삭제 링크를 추가한 코드는 다음과 같다. 
...
```html
 ...
 {% block content %}
   ...
   <p>Written by: {{ post[3] }}</p>
   <pre>{{ post[2] }}</pre>
   <a href="/posts/{{ post[0] }}/edit">Edit Post</a>
   <br>
   <a href="/posts/{{ post[0] }}/delete">Delete Post</a>
   <br>
   <br>
   <a href="/posts">Go to Posts</a>
 {% endblock %}
...
그 다음, *post_delete.html*과 *post_delete_failure.html*은 각각 다음과 같다. 이때, 두 HTML 문서로 나타나는 페이지 형태는 다음과 같다. 

![[Pasted image 20260121001447.png|350]]![[Pasted image 20260121001457.png|350]]
```html
{% extends 'base.html' %}

{% block content %}
  <h2>Post Delete</h2>
  <form action="/posts/{{ post[0] }}/delete" method="post">
...
## 웹 서비스 테스트하기
---
이제, 모든 구현이 끝났으니 지금까지 구현한 게시판을 직접 테스트해보며 기능들이 잘 구현되었는지 확인하는 절차만 남았다.

만약, 코딩의 실수 또는 웹 서버가 잘 작동하지 않는 경우 [이 링크](https://dreamhack-lecture.s3.ap-northeast-2.amazonaws.com/uploads/web-service/flask-app-board.zip)의 zip 파일을 다운로드해 실습하면 되며, 해당 파일은 구성이 끝나 정상적으로 동작하는 웹 서버의 압축 파일이다. 

1. 터미널을 켜서 앞서 구성한 디렉터리의 트리 구조에서 *app.py* 가 위치한 최상위 디렉터리로 이동한 후 `$ python3 app.py` 명령어로 웹 서버를 실행한다. 
	![[Pasted image 20260121003009.png]]
2. 웹 서버가 켜진 후 브라우저에서 `http://127.0.0.1:31337` 주소로 접속하면 로그인 페이지로 리다이렉트된다. 
	![[Pasted image 20260121003051.png]]
3. 이제, 회원가입 기능을 테스트해본다. 로그인 페이지에서 빨간 네모로 표시된 버튼을 누른다.
	![[Pasted image 20260121003138.png]]
4. 회원가입 페이지에서 `Username`과 `Password`에 각각 `user123`, `password`를 입력한 후 **Register** 버튼을 누르면 회원가입이 완료되며, 로그인 페이지로 리다이렉트된다. 
5. 로그인 페이지에서 회원가입한 정보를 그대로 입력한다.
	![[Pasted image 20260121003429.png]]
6. **Login** 버튼을 누르면 로그인이 성공적으로 완료되며 메인 페이지로 이동한다. 이때, 게시글 목록 페이지로 이동할 수 있는 버튼과 로그아웃할 수 있는 버튼 두 개가 존재한다. 
	![[Pasted image 20260121003627.png]]
7. **CRUD 기능은 직접 해 보면서 테스트해보자.**
...
**혹시 몰라, 코드 전체를 Obsidian 내부에 *Web_Develop* 라는 폴더로 넣어 놨으니 로컬 컴퓨터 내에 코드가 없다면 경로를 설정해서 직접 실행해 보자. (초기 설정에는 *webserver.db* 파일은 없는 상태이다)**