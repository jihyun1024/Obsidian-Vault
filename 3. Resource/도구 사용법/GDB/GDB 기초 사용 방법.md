...
간단하게 설명하면, 위의 사진의 경우, 프로그램에 대한 상세한 정보들을 출력해 주는 것으로 현재 실행되고 있는 [[Assembly 메인 문서|어셈블리 명령어]]와 그 주변의 명령어들을 보여준다. 
...
그러나 [[x64dbg 사용법]]처럼 `call`명령어 뒤에 함수를 호출하는 경우, `ni`는 x64dbg의 F8처럼 함수의 내부로 들어가지 않지만, `si`는 x64dbg의 F7처럼 함수의 내부로 들어간다는 차이점이 있다. 
