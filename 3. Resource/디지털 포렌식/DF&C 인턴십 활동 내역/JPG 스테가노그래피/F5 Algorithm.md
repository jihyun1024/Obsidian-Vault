...
따라서, ==요약하면 $n = 2^k -1$에서 $k$는 한 번에 은닉할 메시지 bit 수==이며, $n$은 그 $k$bit를 은닉하기 위해 사용할 계수의 개수로 F5 알고리즘에서 실제로 메시지를 은닉하는 단계는 바로 매트릭스 인코딩 단계이다. 
조금 더 구체적으로 서술하면 
1. 묶음 만들기
	1. JPG의 0이 아닌 DCT 계수들을 $n = 2^k - 1$개씩 묶는다. 
	2. $k = 2$라면 $n = 3$개의 계수를 하나로 묶는다.
2. 현재 상태 계산
	1. 그룹의 계수들의 LSB들을 모아 Hamming Code 원리를 바탕으로 XOR 연산을 수행한다.
3. 삽입할 메시지와 비교
	1. 계산된 값이 은닉하고자 하는 $k$bit 메시지와 같으면 변경하지 않는다.
	2. 계산된 값이 은닉하고자 하는 $k$bit 메시지와 다르면 한 개 계수의 LSB만 변경해 일치하도록 한다. 

즉, Matrix Encoding은 **주어진 묶음의 LSB 패턴이 곧 메시지의 일부($k$bit)를 나타내도록 조정하는 과정**이며, 그 조정을 통해 메시지가 JPG 이미지 속에 은닉된다. 
그래서 메시지 은닉은 퍼뮤테이션으로 위치를 흩뿌린 뒤, 매트릭스 인코딩 단계에서 실제로 이뤄진다고 이해하면 된다.
